/**
 * Synthetic Data Generator for FA Advisor UI
 * Generates realistic hierarchical data:
 * - 4,000 advisors
 * - 800,000 households (200 per advisor)
 * - 5.6M accounts (7 per household)
 * - 84M holdings (15 per account, from S&P 500)
 */
...
const ASSET_CLASSES = ['equity', 'fixed_income', 'cash', 'alternative'];
...
async function generateData() {
  ...
  try {
    ...
    for (let i = 1; i <= CONFIG.ADVISORS; i++) {
      ...
      if (i % 500 === 0) console.log(`  Generated ${i.toLocaleString()} advisors...`);
    }
    ...
    for (const advisorId of advisorIds) {
      ...
      if (householdCount % 10000 === 0) console.log(`  Generated ${householdCount.toLocaleString()} households...`);
    }
    ...
    for (let i = 0; i < householdIds.length; i += CONFIG.BATCH_SIZE) {
      ...
      if (accountCount % 50000 === 0) console.log(`  Generated ${accountCount.toLocaleString()} accounts...`);
    }
    console.log(`✓ Generated ${accountIds.length.toLocaleString()} accounts\n`);

    // Step 5: Generate holdings
    console.log('📈 Generating holdings (this will take a while)...');
    let holdingCount = 0;
    ...
    for (let i = 0; i < accountIds.length; i += CONFIG.BATCH_SIZE) {
      ...
      for (const accountId of batchAccounts) {
        // Select random stocks from S&P 500
      ...
      if (holdingCount % 100000 === 0) console.log(`  Generated ${holdingCount.toLocaleString()} holdings...`);
    }
